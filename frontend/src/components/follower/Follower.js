...
function Follower() {
    ...
    async function profile() {
        ...
        const res = await axios.get(`http://localhost:5000/api/profile/following`, {
            headers: {
                'Content-Type': 'application/json',
                'x-auth-token': token,
            },
        });
        setProfiles(res.data.following);
    ...
    return (
        <section className="container">
            ...
            <h1 className="large text-primary">Following profiles</h1>
            <div>
                <input
                    type="text"
                    className='m-1 p-1'
